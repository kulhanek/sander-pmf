...
#ifdef MPI
use nfe_constants, only : ABMD_REMLOG_UNIT
#endif /* MPI */
...
character(*), private, parameter :: SECTION = 'nfe_abmd'
...
NFE_REAL,           private, save :: driven_cutoff
! Moradi end
!-----------------------------------------------------------------------------
#ifdef MPI
...
character(*), private, parameter :: REMLOG_FILE = 'nfe-abmd.log'
...
! Modified by M Moradi
! Driven ABMD
!subroutine on_force(x, f, wdriven, udriven)
! Moradi end
          ...
          if (multisander_numgroup().gt.1) then
            ...
            if (masterrank.eq.0) then
               ...
               if (abs(w_norm).gt.0) then
                  ...
                  if (selection_epsilon.lt.ONE) then
                     nfe_assert(selection_criterion.ge.ZERO)
                     if (selection_criterion.ge.selection_epsilon*log(NFE_TO_REAL(multisander_numgroup()))) then
                        write (unit= LOG_UNIT , fmt =&
            ...
            if (masterrank+1.ne.all_n(masterrank+1)) then
              ...
              write (unit= LOG_UNIT , fmt ='(a,a,'//pfmt(masterrank+1)//',a,'//pfmt(all_n(masterrank+1))//'/)')&
                 NFE_INFO,'Selestion resampling : new ',masterrank+1,' comes from ',all_n(masterrank+1)
            end if
