...
character(*), private, parameter :: SECTION = 'nfe_bbmd'
...
! Modified by F Pan 
      if (self%pseudo.gt.TINY) then
         t = sander_temp0()
         wt_umbrella = self%umbrella
         call umbrella_wt_mod(wt_umbrella,t,1/self%pseudo)
         call umbrella_save(wt_umbrella, self%wt_umbrella_file)
      end if
! Pan end      
   end if
#endif /* NFE_NO_NETCDF */
