...
! Modified by WJM, YD, RL
   ...
   if (imin == 0) then
      ...
      call adj_mem_ptr( r_ptr, lvel4, 3*natom*am_nbead + mxvar )
      call adj_mem_ptr( r_ptr, lvel5, 3*natom*am_nbead + mxvar )
      call adj_mem_ptr( r_ptr, lvel6, 3*natom*am_nbead + mxvar )
      call adj_mem_ptr( r_ptr, llng,    3*natom*am_nbead + mxvar )
      call adj_mem_ptr( r_ptr, llngold, 3*natom*am_nbead + mxvar )
      call adj_mem_ptr( r_ptr, lvicf, 3*natom*am_nbead + mxvar )
      ...
      call adj_mem_ptr( r_ptr, lvel4, 0 )
      call adj_mem_ptr( r_ptr, lvel5, 0 )
      call adj_mem_ptr( r_ptr, lvel6, 0 )
      call adj_mem_ptr( r_ptr, llng, 0 )
      call adj_mem_ptr( r_ptr, llngold, 0 )
      call adj_mem_ptr( r_ptr, lvicf, 0 )
...
# ifdef MPI
      if(iamoeba.eq.0.and.periodic == 1) then
         ...
         ! AWG - allow for load inbalance for small atom numbers:
         if ( (numtasks > 1) .and. (natom < 10000) ) maxpr = 2*maxpr
      end if
# endif
   end if
   ...
   if( igb == 0 .and. ipb == 0 ) then
      ...
      iendr = lastr
      iendi = i_ptr
      call debug_mem(natom,istartr,iendr)
      lastr = iendr
      i_ptr = iendi
      lasti = i_ptr
