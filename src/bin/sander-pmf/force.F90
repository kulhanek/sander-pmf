...
#  endif
#  ifdef MPI
  use nfe_sander_proxy, only: nfe_remember_initremd => remember_initremd, &
                              infe
#  else
...
#include "flocntrl.h"
  ...
  if (infe == 1) then
#  ifdef MPI
    call nfe_remember_initremd(rem .gt. 0 .and. mdloop .eq. 0)
#  endif
    call nfe_on_force(x, f)
  end if
#endif
...
!    pot%scf = escf
  ...
  ! End Semi-Empirical Bon-Oppenheimer MD (SEBOMD) computations
