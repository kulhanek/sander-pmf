...
subroutine runmd(xx,ix,ih,ipairs,x,winv,amass,f, &
   ...
   !  Runmd operates in kcal/mol units for energies, amu for masses,
   ...
   use cmd_vars, only: activate, file_pos_cmd, file_vel_cmd,  &
   ...
   use trace
   use stack
   use decomp, only : decpr, jgroup, indx, irespw, &
#ifdef MPI
   ! -- decomp MPI (e.g., ti decomp)
                      nrs, collect_dec, &
#endif
                      checkdec, printdec
   use fastwt
   ...
   use nose_hoover_module, only: Thermostat_switch,  &
                                 Thermostat_integrate_1, Thermostat_integrate_2, &
...
!   ...and related distribution functions:
     ...
     _REAL_ argvel(-lep:lep),distrs(-lep:lep), &
                      distrh1(lep),distrh2(-lep:lep),distrh3(-lep:lep)
#ifdef MPI
     _REAL_ sdistrs(-lep:lep),sdistrh1(lep), &
                      sdistrh2(-lep:lep),sdistrh3(-lep:lep)
#endif
...
! gfortran 4.2 and lower
   ...
   _REAL_ tspan,atempdrop,fln,scaltp  !,scaltpo for old (pre-amber7) T-scaling
...
! Variables and parameters for constant surface tension:
   ...
   logical qsetup
   _REAL_, allocatable, dimension(:) :: for
#ifdef RISMSANDER
   logical irismdump
#  ifdef RISM_DEBUG
...
! END PLUMED RELATED VARIABLES
   ...
   if (.not. allocated(for)) allocate(for(nr3))
   ...
   if(abfqmmm_param%abfqmmm == 1) then
      ...
      if(abfqmmm_param%system == 1) then
         if(abfqmmm_param%qmstep == 1) abfqmmm_param%v(1:nr3+iscale) = v(1:nr3+iscale)
         v(1:nr3+iscale) = 0.d0
         t = t+dt
         if(abfqmmm_param%maxqmstep == 0) t = 0
      else
   ...
   if (master) then
      ...
      if (abfqmmm_param%abfqmmm /= 1 .or. &
         ...
         write (6,'(a,f8.0,a,i6,a,f8.0)') &
               "|  NDFMIN = ",rndfp, "     NUM_NOSHAKE = ",num_noshake, &
               "     CORRECTED RNDFP = ", rndfp
         write (6,'(a,f8.0)') &
...
! also ndfmin is not always being handled.
   ...
   if(abfqmmm_param%abfqmmm /= 1) then
      ...
      if ( ntt==4 .and. abfqmmm_param%qmstep == 1 .and. abfqmmm_param%system == 1 ) &
...
#  endif
   ...
   if( init == 3 .or. nstlim == 0 .or. &
      ...
      for(1:nr3) = f(1:nr3)
#ifdef MPI
      call xdist(for,xx(lfrctmp),natom)
#endif

      if(abfqmmm_param%abfqmmm == 1) then

         if(abfqmmm_param%system == 1) abfqmmm_param%f1(1:nr3) = for(1:nr3)

         if(abfqmmm_param%system == 2) then
            abfqmmm_param%f2(1:nr3) = for(1:nr3)
...
#  endif
   ...
   if (init /= 4 .or. nstlim == 0 .or. &
      ...
      if ( (nstep <= 0 .and. master .and. facc /= 'A') .or. &
...
#endif /* LES */
         if(abfqmmm_param%abfqmmm /= 1 .or. &
               abfqmmm_param%system == 1 .or. nstep == 0) &
      ...
      if (abfqmmm_param%abfqmmm == 1 .and. abfqmmm_param%system == 1) then
         deallocate(for, stat=ierr)
         REQUIRE(ierr==0)
         return
      end if
      if (nstlim == 0) then
         if(abfqmmm_param%abfqmmm == 1) v(1:nr3) = abfqmmm_param%v(1:nr3)
#ifdef MPI
         ...
         if(master) then 
#endif
            if(ntwr>0) call mdwrit(nstep,nr,ntxo,ntb,x,v,t,temp0)
            if(ntwx>0) call corpac(x,1,nrx,MDCRD_UNIT,loutfm)
            if(ntwv>0) call corpac(v,1,nrx,MDVEL_UNIT,loutfm)
            if(ntwf>0) call corpac(for,1,nrx,MDFRC_UNIT,loutfm)
...
! 03132009            if( i_qi == 2 ) call qi_corrf_les ( cartpos, amass )
   ...
   if(abfqmmm_param%abfqmmm == 1) then
      ...
      call abfqmmm_combine_forces()
#ifdef MPI
      call mpi_bcast(abfqmmm_param%f, 3*natom, mpi_double_precision, 0, commsander, ierr)
#endif 
      for(1:nr3) = abfqmmm_param%f(1:nr3)
      f(1:nr3) = abfqmmm_param%f(1:nr3)
   end if
...
#  endif
   ...
   if ( qmmm_nml%vsolv > 1 ) then
      ...
      call adaptive_qmmm(nstep,natom,x,f,ener%pot%tot, ntpr, ntwx, &
...
#  endif
      ...
      do j=istart,iend
         ...
         aamass = amass(j)
#  ifdef LES
         if (temp0les >= 0 .and. temp0 /= temp0les .and. cnum(j) /= 0 ) then
...
#  endif
   ...
   ifdump = .false.             ! Write forces this step?
   ivscm  = .false.             ! Do center of mass removal this step?
#ifdef RISMSANDER
   ...
   total_nstep=nstep+1
   total_nstlim=nstlim
   if(abfqmmm_param%abfqmmm == 1) total_nstep=abfqmmm_param%qmstep
     ...
     !else if (ntp > 2) then
      ...
      if (ntp > 0) then
         ...
         if (icfe /= 0 .and. ifsc == 1) then
            if (master) then
               call sc_pscale(x,amass,nspm,nsp,oldrecip,ucell)
            end if
...
!
      ...
      if (ifdump .and. (abfqmmm_param%abfqmmm == 1)) &
         call corpac(for,1,nrx,MDFRC_UNIT,loutfm)
      ...
      if (lout) then
         ...
         if (ipimd.eq.0 .and. ntt > 4 .and. ntt <= 8 ) then
...
#  endif /*MPI*/
   ...
   if (plumed /= 0) call plumed_f_gfinalize()
...
! Variables and parameters for constant surface tension:
   ...
   integer idumar(4)
#ifdef RELAXATION_TRAJ
   ! DEBUG -- RELAXATION_TRAJ will print the relaxation dynamics to a trajectory
   ! file
