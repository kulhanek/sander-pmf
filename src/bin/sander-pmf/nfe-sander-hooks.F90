...
subroutine on_delta(o_masterrank, need_U_xx, U_mm, U_mo, U_om, U_oo)
   ...
   use nfe_stsm_hooks, only : stsm_on_delta => on_delta
   ...
   call stsm_on_delta(o_masterrank, need_U_xx, U_mm, U_mo, U_om, U_oo)
...
subroutine on_exchange(o_masterrank)
   ...
   use nfe_stsm_hooks, only : stsm_on_exchange => on_exchange
   ...
   call stsm_on_exchange(o_masterrank)
...
#  include "nfe-mpi.h"
...
#ifdef MPI
      if (multisander_rem().lt.3 .and. multisander_rem().gt.0) then
         ...
         if (my_mdin.ne.sander_mdin_name()) then
            ...
            if (ios > 0) then
               write (unit = ERR_UNIT, fmt = '(/a,a,a,a/)') &
                  NFE_ERROR, 'failed to open ''', trim(my_mdin), &
                  ''' for reading'
               call terminate()
            end if
         else
   ...
   call stsm_on_sander_init(my_mdin, my_unit, amass, my_idx)
...
#else
   call smd_on_sander_init(5, amass, acrds)
   call pmd_on_sander_init(5, amass)
   call abmd_on_sander_init(5, amass)
   call stsm_on_sander_init(5, amass)
#endif /* MPI */
...
!   call smd_on_force(x, f, wdriven, udriven)
   call smd_on_force(x, f)
   call pmd_on_force(x, f)
!   call abmd_on_force(x, f, wdriven, udriven)
   call abmd_on_force(x, f)
   call stsm_on_force(x, f)
#ifdef NFE_ENABLE_BBMD
!   call bbmd_on_force(x, f, wdriven, udriven)
   call bbmd_on_force(x, f)
#endif /* NFE_ENABLE_BBMD */
! Moradi end
