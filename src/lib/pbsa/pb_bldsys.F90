...
subroutine pb_exmol( verbose,ifcap,mpdec,ipb,inp,savbcopt,saopt,sasopt,natom,&
   ...
   use poisson_boltzmann, only: ligand, &
...
#  include "pb_md.h"
   ...
   if ( memopt > 0 .and. sasopt == 0 ) then
      ...
      call sa_driver(verbose,.true.,pqropt,ipb,inp,natom,natom,dosas,ndosas,npbstep,nsaslag,&
         ligand, outflag,&
   ...
   ! part c.
   ! RL: set up irregular/boundary grid nodes info
