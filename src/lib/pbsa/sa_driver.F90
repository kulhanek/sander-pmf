...
module solvent_accessibility
   ...
   _REAL_, allocatable   ::   mdsig(:)    ! atomic radii (np vdw radii) equals to radi if read in
   _REAL_, allocatable   ::    rmin(:)    ! atomic radii (np vdw radii) equals to radi if read in
...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
!+ Cleanup routine of solvent accessible surface and arcs for the multi-block focussing
subroutine sa_free_mb ( dosas,ndosas )
   ...
   if ( dosas .eq. ndosas ) then
      ...
      if ( alloc_err(1)+alloc_err(2)+alloc_err(3)+alloc_err(4)+alloc_err( 5)+&
         ...
         write(6,'(a,21i6)') 'SA Bomb in sa_free_mb(): DeAllocation aborted', alloc_err(1:21)
         call mexit(6, 1)
      end if
   end if
...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
!+ Setting up radi for SAS/SES/VDW(i.e. = SAS with prob=0, used by both PB and NP
...
#  include "pb_constants.h"
   ...
   !      r(iatm) = 1.81d0
   !   else
   !      write(6,'(a)') 'SA Bomb in sa_init(): No radius assigned for atom', iatm, isymbl(iatm)
   !      call mexit(6, 1)
   !   end if
   !end do
...
subroutine phi_aaradi( natom, isymbl, radi )
   ...
   do iatm = 1, natom
      if      (isymbl(iatm)(1:2) == 'Br' .or. isymbl(iatm)(1:2) == 'br') then
         ...
         write(6,'(a,2i6)') 'SA Bomb in phi_aaradi(): No radius assigned for atom', iatm, isymbl(iatm)
         call mexit(6, 1)
      end if
   end do
...
#  include "pb_constants.h"
   ...
   do iatm = 1, natom
      ...
      if ( radi(iatm) == ZERO ) then
         write(6,'(a,i6,2(2x,a4))') 'PB Bomb in pb_aaradi(): &
               &No radius assigned for atom', iatm, igraph(iatm), isymbl(iatm)
         call mexit(6, 1)
      endif
...
#  include "pb_constants.h"
   ...
   ! for regular Amber files input only

   if ( (.not. ligand) .and. done_sa_driver ) then
      write(6,'(a)') "sa_driver() is not allowed to be called twice without ligand/mb."
      call mexit(6,1)
   end if
   ...
   if ( dosas .eq. ndosas ) then
      ...
      if ( .not. done_sa_driver ) then
          ...
          call sa_srf( verbose, pbprint, pqropt, natom, 1, atmlast )
      ...
      if ( .not. ligand .or. done_sa_driver ) then
         ...
         if ( .not. npsa .and. (ipb == 1 .or. ipb == 2 .or. ipb == 4 .or. ipb == 5) ) &
         call sa_arc( verbose,pbprint,pqropt,natom,1,atmlast )
...
10    continue
   ...
   ! for pqr files, we don't have the excluded list, so all sphere
   ...
   do iatm = atmfirst, atmlast
      ...
      do isph = 1, maxsph
         nsrf = nsrf + 1
         sphcrd(1,nsrf) = radip(iatm)*scrd(1,isph)
         sphcrd(2,nsrf) = radip(iatm)*scrd(2,isph)
         sphcrd(3,nsrf) = radip(iatm)*scrd(3,isph)
      end do
...
subroutine sa_arc( verbose,pbprint,pqropt,natom,atmfirst,atmlast )
   ...
   allocate( triatm1(3,   maxtri*natom), stat = alloc_err(7) )
   allocate(  triarc(3,   maxtri*natom), stat = alloc_err(8) )
   allocate( triarc1(3,   maxtri*natom), stat = alloc_err(9) )
   allocate(  tricrd(3,   maxtri*natom), stat = alloc_err(10))
   ...
   allocate( triatm1(3,              1), stat = alloc_err(7) )
   allocate(  triarc(3,              1), stat = alloc_err(8) )
   allocate( triarc1(3,              1), stat = alloc_err(9) )
   allocate(  tricrd(3,              1), stat = alloc_err(10))
   ...
   if ( alloc_err(1)+alloc_err(2)+alloc_err(3)+alloc_err(4)+alloc_err(5)+&
        alloc_err(6)+alloc_err(7)+alloc_err(8)+alloc_err(9)+alloc_err(10)+&
        alloc_err(11)+alloc_err(12) /= 0 ) then
   ...
   if ( alloc_err(1)+alloc_err(2)+alloc_err(3)+alloc_err(4)+alloc_err(5) /= 0 ) then
      write(6,'(a,5i6)') 'SA Bomb in sa_arc(): Allocates aborted 3', alloc_err(1:5)
      call mexit(6, 1)
   end if
   ...
   call circle( natom,atmfirst,atmlast,ksrf,arcres,fstsph1,lstsph1,spharc1,sphcrd1, &
   ...
   call exclud( atmfirst,atmlast,4,1,ksrf,lsrf,fstsph1,lstsph1,fstsph,lstsph,spharc1,spharc,sphcrd1,sphcrd, &
   ...
   if ( alloc_err(1)+alloc_err(2) /= 0 ) then
      write(6,'(a,2i6)') 'SA Bomb in sa_arc(): Allocation aborted', alloc_err(1:2)
      call mexit(6, 1)
   end if
   call pbtimer_stop(PBTIME_PBSAARC_SETUP)
...
!        write(6,'(a)') 'trimer',triatm(1:3,isph)
      end do
   end if
   ...
   deallocate( triatm1, stat = alloc_err(6) )
   deallocate( triarc1, stat = alloc_err(7) )
   deallocate(  tricrd, stat = alloc_err(8) )
   deallocate( tricrd1, stat = alloc_err(9) )
   deallocate(knocktri, stat = alloc_err(10))
   if ( alloc_err(1)+alloc_err(2)+alloc_err(3)+alloc_err(4)+alloc_err(5) + &
        alloc_err(6)+alloc_err(7)+alloc_err(8)+alloc_err(9)+alloc_err(10) /= 0 ) then
      write(6,'(a,10i6)') ' SA Bomb in sa_arc(): Deallocation aborted', alloc_err(1:10)
      call mexit(6, 1)
   end if
   call pbtimer_stop(PBTIME_PBSAARC_SETUP)
   ...
   if ( verbose .and. pbprint ) then
      ...
      write(6,'(2(a,i12),/a,e11.6)') &
             ...
             ' Number of trimer arc points exposed', ntri, &
...
!              iscycle = .false.
      ...
      if ( marc(iatm) > maxarc ) then
         write(6,'(a,2i6)') 'SA Bomb in circle(): Stored surface arcs over limit', iatm, marc(iatm)
         call mexit(6,1)
      end if
...
!     write(6,'(a)') "u3",u(3,1:3)
!     write(6,'(a)') "b",b(1:3)
!     write(6,'(a)') "det",det,t3
!     stop
!  end if
...
!     write(6,'(a)') "inconsistent",d1,d2,t2
!  end if
   if ( d1 <= d2 ) then
      isexist = .false.
      return
