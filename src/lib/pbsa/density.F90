...
subroutine density_lvlset(memopt,outlvlset,outmlvlset,natom,xm,ym,zm,gox,goy,goz,h,&
   ...
   insas(0:xm+1,0:ym+1,0:zm+1) = int(sign(1.001d0, lvlset(0:xm+1,0:ym+1,0:zm+1) - 1.0d0))
...
subroutine bndatm(nbnd,natom,xm,ym,zm,h,dprobh,cutfd,gcrd,radi,iepsav,lvlset)
   ...
   maxlst =16*nint( dble(natom)*(sqrt(cutfd)**3/3.0d0+10.d0) )
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   ...
   mzctr = (mzmin + mzmax)*0.5d0; mthick2 = (mzmax - mzmin)*0.5d0
   lowk = max(0,ceiling(mzmin-2.0*dprobh)); highk = min(zm+1,floor(mzmax+2*dprobh))
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   ...
   do i = 0, xm+1
      ...
      dx = i - poredata(1,k)
      dy = j - poredata(2,k)
      cdist = sqrt(dx**2.0d0 + dy**2.0d0) - poredata(3,k)
      ...
      if ( ddist < 0 ) then
         ...
         if ( cdist <= 0 ) then
            ...
            ddist = sqrt(ddist**2.0d0 + cdist**2.0d0)
         end if
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   ...
   do j = 0, ym+1
   do i = 0, xm+1
      if ( dist(i,j,k) <= 1.0d0 ) then
         if ( dist(i,j,k) <= 0.0d0 ) then
...
subroutine irreg_grd(xm,ym,zm,nbnd,h,iepsav,phi)
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
...
end subroutine irreg_grd
#ifdef NEWINTERP
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
!+ set up jump conditions at irregular grid points
subroutine irreg_jump
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
! set up irregular grid point jump conditions and their first and second
! derivatives. Of course these are at the grid points' projection points on the
! interface.
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   ...
   ! setting up the jump conditions at the projection points of the irreular
   ! (boundary) grid points

   call prodis(xm,ym,zm,nbnd,bcopt,bi,bo,atmfirst,atmlast,nbnd,cirreg,wp,qp)
   !do i = 1, nbnd
   ...
   call coed20(xm,ym,zm,h,h,h,h,gox,goy,goz,nbnd,nbnd,nq,index2,cirreg, &
               wcoe,wxcoe,wycoe,wzcoe,wxxcoe,wyycoe,wzzcoe,wxycoe,wxzcoe,wyzcoe)
   call qint(xm,ym,zm,h,h,h,h,gox,goy,goz,nbnd,nq,index,index2,cirreg,qp, &
             wcoe,wycoe,wzcoe,q0p,qyp,qzp)
   call wint(xm,ym,zm,h,h,h,h,gox,goy,goz,nbnd,nq,index,index2,cirreg,wp, &
             wcoe,wycoe,wzcoe,wyycoe,wzzcoe,wyzcoe,w0p,wyp,wzp,wyyp,wzzp,wyzp)

   ! step 2: this is the second derivatives

   call coed6(xm,ym,zm,h,h,h,h,gox,goy,goz,nbnd,nbnd,nq,index2,cirreg, &
              wcoe,wxcoe,wycoe,wzcoe,wxxcoe,wyycoe,wzzcoe,wxycoe,wxzcoe,wyzcoe)
   call wint(xm,ym,zm,h,h,h,h,gox,goy,goz,nbnd,nq,index,index2,cirreg,wp, &
             wcoe,wycoe,wzcoe,wyycoe,wzzcoe,wyzcoe,w0p,sss1,sss2,wyyp,wzzp,wyzp)
   !write(200,*)
   !wcoe,wycoe,wzcoe,q0p,qyp,qzp,wyycoe,wzzcoe,wyzcoe,w0p,wyp,wzp,wyyp,wzzp,wyzp
   !write(200,*) q0p,qyp,qzp,w0p,wyp,wzp,wyyp,wzzp,wyzp

end subroutine irreg_jump
#endif
