...
#  include "pb_constants.h"
   ...
   do i = 1, natom
      ...
      do j = 1, natom
         ...
         ! even if there is no excluded list, the two loops
         ! below should just skip since nshrt is all zero.
...
!write(2012,*) i,nclose,cntr, '|'
      ...
      ! eclose is zero if pqropt == 0 since there is no topology info read in
...
subroutine pb_setgrd( ipb, verbose, pbprint, initial, pbgrid, ifcap, atmfirst, atmlast, xcap, ycap, zcap, cutcap )
   ...
   if ( .not. initial ) then
      ...
      deallocate(     xs, stat = alloc_err(21) )
      if ( sum(alloc_err(1:32)) /= 0 ) then
        write(6,'(a)') 'PB bomb in pb_setgrd(): Deallocation aborted', alloc_err(1:32)
         call mexit(6, 1)
      end if
   end if
...
#  include "extra.h"
   ...
   if ( verbose .and. pbprint ) then
      write(6,'(a)')
      write(6,'(a)')
      write(6,'(a)') '======== Setting up Grid Parameters ========'
