...
#include "def_time.h"
  ...
  private uvParticleMeshEwaldPotentialFortran
...
contains
  ...
  subroutine rism3d_potential_setcut(this, cut)
    ...
    ! tcut                  : trial cutoff
    _REAL_ :: exp_quarter_xappa2_smear2, tcfLongRangeAsympR_coeff_at_cut, dcfLongRangeAsympR_coeff_at_cut,&
  ...
  subroutine rism3d_potential_calc(this)
    ...
    if (ubound(this%uuv, 1) /= this%grid%localDimsR(1) .or. &
       ...
       ! this%uuv1d_T => safemem_realloc(this%uuv1d_T, this%grid%totalLocalPointsK, &
       !      1, o_preserve = .false., o_aligned = .true.)
    end if
    ...
    if (this%solute%charged) then
       if (this%periodic) then
          if (this%periodicPotential == 'pme') then
             ...
             ! call rism3d_opendx_write("rism3d_potential_cpp_pme.dx", this%uuv(:,:,:,1), &
             !      this%grid, this%solute)
             ! print *, "Fortran PME time!"
             ! this%uuv = 0
             call uvParticleMeshEwaldPotentialFortran(this, this%uuv)
             ! call rism3d_opendx_write("rism3d_potential_fort_pme.dx", this%uuv(:,:,:,1), &
             !      this%grid, this%solute)
             ! call exit(0)
          else
             ! print *, "Ewald sum time!"
             call uvEwaldSumPotential(this, this%uuv)
             ! call rism3d_opendx_write("rism3d_potential_ewald.dx", this%uuv(:,:,:,1), &
             !      this%grid, this%solute)
             ! call exit(0)
          end if
       else
  ...
  subroutine  rism3d_potential_dcf_tcf_long_range_asymptotics(this)
    ...
    if ((.not. this%periodic) .or. (this%periodicPotential == 'ewald')) then
       ...
       !JAJ This is the long range portion of the solute electrostatic periodic field!
       ! See Understanding Molecular Simulation 2E, eq. 12.1.15 on p. 297.
       ! Note: the result is divided by volume since FFTW does not do this for you
       ! during transforms, though technically this is not part of the frequency space
       ! potential.
       ...
       do ig = ig0, this%grid%totalLocalPointsK / 2
          ...
          do iu = 1, this%solute%numAtoms
             ...
             if (this%periodic) then
                ...
                ! if (.not. printedImages) then
                !    print *, "before: ", solutePosition
                !    print *, "after: "
                !    print *, dot_product(solutePosition, this%grid%unitCellUnitVectorsR(1,:))
                !    print *, dot_product(solutePosition, this%grid%unitCellUnitVectorsR(2,:))
                !    print *, dot_product(solutePosition, this%grid%unitCellUnitVectorsR(3,:))
                ! end if
       ...
       ! if (this%periodic) then
       !    call rism_timer_end(this%coulombLongRangeTimer)
       !    call timer_end(TIME_UCOULULR)
       ! end if
    end if
    ...
    if (power == 2) then
       ...
       do ishell = 1, maxshell
          ...
          if (this%grid%mpirank == 0) then
             do ix = -ishell, this%grid%globalDimsR(1) - 1 + ishell, this%grid%globalDimsR(1) - 1 + 2 * ishell
                do iy = -ishell, this%grid%globalDimsR(2) - 1 + ishell, this%grid%globalDimsR(2) - 1 + 2 * ishell
                   do iz = -ishell, this%grid%globalDimsR(3) - 1 + ishell, this%grid%globalDimsR(3) - 1 + 2 * ishell
                      r(1) = ix * this%grid%spacing(1)
                      r(2) = iy * this%grid%spacing(2)
                      r(3) = iz * this%grid%spacing(3)
                      shell = shell + tcf_long_range_asymptotics_R(this, r)**power
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine mixSoluteSolventLJParameters(this)
    ...
    do iv = 1, this%solvent%numAtomTypes
       do iu = 1, this%solute%numAtoms
          this%ljSigmaUV(iu, iv) = this%solute%ljSigma(iu) + this%solvent%ljSigma(iv)
          this%ljEpsilonUV(iu, iv) = sqrt(this%solute%ljEpsilon(iu) * this%solvent%ljEpsilon(iv))
       end do
    end do
    ...
    do iu = 1, this%solute%numAtoms
       ...
       do id = 1, 3
          ...
          if (closestPoint(id) > 0) then
             first(id) = min(max(1, closestPoint(id) - cutoff(id)), this%grid%localDimsR(id))
             ...
             first(id) = min(max(1, closestPoint(id) - &
                  min(huge(1) + closestPoint(id), cutoff(id))), this%grid%localDimsR(id))
             last(id) = max(min(this%grid%localDimsR(id), &
                ...
                if (ucu(igx, igy, igz, 1) == untouched) then
                   ...
                   if (r2 < this%cutoff2) then
                      ...
                      do iu2 = 1, this%solute%numAtoms
                         ...
                         ucu(igx, igy, igz, 1) = ucu(igx, igy, igz, 1) + this%solute%charge(iu2) / ra
                      end do
                   end if
                end if
             end do
  ...
  subroutine uvEwaldSumPotential(this, ucu)
...
#ifdef RISM_DEBUG
    write(6,*) 'Tabulating Ewald sum potential field ...'
    call flush(6)
#endif
...
#if 0
    ! Debug DX output.
    call rism3d_opendx_write( &
         "ewaldpot.dx", this%uuv(:,:,:,1), this%grid, &
         this%solute)
#endif
    ...
    !    do igy = 1, this%grid%localDimsR(2)
    !       do igx = 1, this%grid%localDimsR(1)
    !          total = total + ucu(igx, igy, igz, 1)
    !       end do
    !    end do
    ! end do
    ! print *, 'Total Ewald sum potential:', total
  ...
  !> Calculate the periodic Particle Mesh Ewald potential using a C++
  !! implementation. See C++ source for further documentation.
  !! Currently all PME parameters are hardcoded within this
  !! subroutine.
#if 0
  subroutine uvParticleMeshEwaldPotential(this, ucu)
    ...
    ! _REAL_ :: ucu_C(this%grid%globalDimsK(3), &
    !      this%grid%globalDimsK(2), &
    !      this%grid%globalDimsK(1))
    _REAL_, pointer :: ucu_C(:, :, :) => NULL()
    ...
    _REAL_, parameter :: beta = 1
    ! Order of b-spline interpolation.
    integer, parameter :: order = 4

    ucu_C => safemem_realloc(ucu_C, &
         this%grid%globalDimsK(3), &
         this%grid%globalDimsK(2), &
         this%grid%globalDimsK(1), &
         o_preserve = .false., o_aligned = .true.)

    call pmepot(this%solute%numAtoms, &
         this%solute%position, this%solute%charge, &
         this%grid%globalDimsR, &
         this%grid%boxLength, &
         this%grid%boxVolume, &
         this%grid%unitCellUnitVectorsR, &
         this%grid%unitCellVectorsR, &
         this%grid%unitCellVectorsK, &
         beta, order, this%cutoff, &
         ucu_C(:,:,:))

    ! Transpose C++ row-major array to Fortran column-major order.
    !TODO: This is an expensive operation for large arrays, but for
    ! some reason the libfftw plan creation in C++ segfaults when
    ! column-major order is used.
    do igz = 1, this%grid%globalDimsR(3)
       do igy = 1, this%grid%globalDimsR(2)
          do igx = 1, this%grid%globalDimsR(1)
             ucu(igx, igy, igz, 1) = ucu_C(igz, igy, igx)
             ! ucu(igx, igy, igz, 1) = ucu_C(igx, igy, igz)
          end do
       end do
    end do
    ...
    ! Debug DX output.
    call rism3d_opendx_write( &
         "pmepot.dx", this%uuv(:,:,:,1), this%grid, &
         this%solute)
#endif
    ...
    print *, 'Total PME potential:', total
#endif
...
#endif /* uvParticleMeshEwaldPotential */

  !> Long-range portion of the Particle Mesh Ewald (PME) electric potential.
  !! TODO: Document me!
  subroutine uvParticleMeshEwaldPotentialFortran(this, ucu)
    ...
    ! numGridPointsK = this%grid%localDimsR(3) * this%grid%localDimsR(2) * gridDimX_k
    ! numGridPointsK = (this%grid%localDimsR(3) / 2 + 1) &
    !      * this%grid%localDimsR(2) * this%grid%localDimsR(1)
...
# if 0
    print *, "localPtsK", localPtsK
    print *, "local_N and local_k_offset:", local_N, local_k_offset
    ! print *, "localPtsK_y and offset:", localPtsK_y, localOffsetK_y
    print *, "this%grid%localDimsR(3), this%grid%offsetR:", &
         this%grid%localDimsR(3), this%grid%offsetR(3)
# endif
#else
    ...
    !FIXME: Unlike rest of PME implementation this uses x-axis halving
    ! (as it should be in Fortran), not z-axis (as in C++).
    ! do iz = 0, this%grid%localDimsR(3) - 1
    !    lgz = merge(iz, -(this%grid%localDimsR(3) - iz), &
    !         iz < this%grid%localDimsR(3) / 2 + 1)
    !    do iy = 0, this%grid%localDimsR(2) - 1
    !       lgy = merge(iy, -(this%grid%localDimsR(2) - iy), &
    !            iy < this%grid%localDimsR(2) / 2 + 1)
    !       do ix = 0, gridDimX_k - 1
    !          lgx = ix
    !          igk = 1 + ix + (iy + iz * this%grid%localDimsR(2)) &
    !               * gridDimX_k
    !          waveVector = &
    !               2d0 * pi * (this%grid%unitCellVectorsK(1,:) * lgx &
    !               + this%grid%unitCellVectorsK(2,:) * lgy &
    !               + this%grid%unitCellVectorsK(3,:) * lgz)
    !          k2i(igk) = dot_product(waveVector, waveVector)
    !       end do
    !    end do
    ! end do
...
! #if defined(MPI)
!     !DEBUG: Check if smeared charge is the same for various num MPI processes.
!     ! Correct for incorrect stride on 1st dim (L+2).
!     do k = 0, local_N - 1
!        do j = 0, M - 1
!           do i = 0, L - 1
!              outr_1d(1 + i + (j + k * M) * L) = outr_1d(1 + i + (j + k * M) * (L+2))
!           end do
!        end do
!     end do
!     if (this%grid%mpirank == 0) then
...
!     ! call mpi_gather(inr, &
!     call mpi_gather(uuv1d_r, L * M * local_N, MPI_DOUBLE_PRECISION, &
!          uuv1d_final, L * M * local_N, MPI_DOUBLE_PRECISION, &
!          0, this%grid%mpicomm, ierr)
!     if (this%grid%mpirank == 0) then
!        write(filename, "(A24, I1)") '/home/jesse/rism/charge_', this%grid%mpisize
!        open(unit=100, file=filename, access='sequential', status='replace')
!        write(100, "(f30.10, /)", advance='no') sum(uuv1d_final)
!        do igz = 1, N
!           do igy = 1, M
!              do igx = 1, L
!                 write(100, "(f30.10, /)", advance='no') uuv1d_final(igx, igy, igz)
!              end do
!           end do
!        end do
!        close(100)
!     end if
! #else
!     open(unit=100, file='/home/jesse/rism/charge_serial', &
!          access='sequential', status='replace')
!     write(100, "(f30.10, /)", advance='no') sum(uuv1d_r)
!     do igz = 1, N
!        do igy = 1, M
!           do igx = 1, L
!              write(100, "(f30.10, /)", advance='no') uuv1d_final(igx, igy, igz)
...
! #endif
...
#if defined(MPI)
    !TODO: This may be necessary if it is not already done by above loop.
    ! inr = inr / this%grid%totalLocalPointsR

    ! Transpose before transform.
    ! call dcopy(product(this%grid%localDimsR), inr, 1, this%uuv1d, 1)
    ! do igz = 0, this%grid%localDimsR(3) - 1
    !    do igy = 0, this%grid%localDimsR(2) - 1
    !       do igx = 0, this%grid%localDimsR(1) - 1
    !          igk_fort = 1 + igx + (igy + igz * this%grid%localDimsR(2)) &
    !               * this%grid%localDimsR(1)
    !          igk_cpp = 1 + igz + (igy + igx * this%grid%localDimsR(2)) &
    !               * this%grid%localDimsR(3)
    !          inr(igk_fort) = this%uuv1d(igk_cpp, 1)
    !       end do
    !    end do
    ! end do
    ...
    ! call dcopy(numGridPointsK, outk, 1, this%uuv1d, 1)
    ! do igz = 0, this%grid%localDimsR(3) / 2 + 1 - 1
    !    do igy = 0, this%grid%localDimsR(2) - 1
    !       do igx = 0, this%grid%localDimsR(1) - 1
    !          igk_fort = 1 + igx + (igy + igz * this%grid%localDimsR(2)) &
    !               * this%grid%localDimsR(1)
    ...
    !          outk(igk_fort) = cmplx( &
    !               this%uuv1d(1 + (igk_cpp - 1) * 2, 1), &
    !               this%uuv1d(2 + (igk_cpp - 1) * 2, 1))
    !       end do
    !    end do
    ! end do
#else
    ...
    do igz = 0, local_N - 1
       do igy = 0, M - 1
          do igx = 0, (L / 2 + 1) - 1
             ...
             !      + (igy + (igz + this%grid%offsetR(3)) * M) * (L / 2 + 1)
...
#if defined(MPI)
    ! ! Account for fftw_mpi_execute_dft_c2r outputting data with wrong
    ! ! stride (on fastest varying dim).
    ! do igz = 0, local_N - 1
    !    do igy = 0, M - 1
    !       do igx = 0, L - 1
    !          outr_1d(1 + igx + (igy + igz * M) * L) &
    !               = outr_1d(1 + igx + (igy + igz * M) * (L + 2))
    !       end do
    !    end do
    ! end do

    ! !DEBUG: Check that FFTW-transformad potential is same for various
    ! ! num MPI processes.
    ! if (this%grid%mpirank == 0) then
    !    uuv1d_final = 0
    ! end if
    ! ! call mpi_gather(outr, &
    ! call mpi_gather(uuv1d_r, L * M * local_N, MPI_DOUBLE_PRECISION, &
    !      uuv1d_final, L * M * local_N, MPI_DOUBLE_PRECISION, &
    !      0, this%grid%mpicomm, ierr)
    ! if (this%grid%mpirank == 0) then
    !    write(filename, "(A29, I1)") '/home/jesse/rism/uuv1d_final_', this%grid%mpisize
    !    open(unit=100, file=filename, access='sequential', status='replace')
    !    write(100, "(f30.10, /)", advance='no') sum(uuv1d_final)
    !    do igz = 1, N
    !       do igy = 1, M
    ...
    do igz = 0, this%grid%localDimsR(3) - 1
       do igy = 0, this%grid%localDimsR(2) - 1
          do igx = 0, this%grid%localDimsR(1) - 1
             igk_cpp = 1 + igz + (igy + igx * this%grid%localDimsR(2)) &
                  * this%grid%localDimsR(3)
             ucu(igx + 1, igy + 1, igz + 1, :) = inr(igk_cpp)
          end do
       end do
    end do
#else
       ...
       do igy = 0, this%grid%localDimsR(2) - 1
          do igx = 0, this%grid%localDimsR(1) - 1
             igk_cpp = 1 + igz + (igy + igx * this%grid%localDimsR(2)) &
                  * (this%grid%localDimsR(3) / 2 + 1)
             igk_fort = 1 + igx + (igy + igz * this%grid%localDimsR(2)) &
                  * (this%grid%localDimsR(1) / 2 + 1)
             ! ucu(igx + 1, igy + 1, 2 * igz + 1, :) = real(outk(igk_cpp))
             ! ucu(igx + 1, igy + 1, 2 * igz + 2, :) = aimag(outk(igk_cpp))
             ucu(igx + 1, igy + 1, 2 * igz + 1, :) = real(outk(igk_fort))
             ucu(igx + 1, igy + 1, 2 * igz + 2, :) = aimag(outk(igk_fort))
          end do
       end do
    end do
#endif
    ...
    if (safemem_dealloc(kxi) /= 0) then
       call rism_report_error("uvParticleMeshEwaldPotentialFortran: Failed to deallocate arrays.")
    end if
  ...
  end subroutine uvParticleMeshEwaldPotentialFortran
...
#if 0
    ! Signed multiple of unit cell lengths to the nearest image.
    integer :: units

    ! 1. Project position to unit cell vector.
    ! 2. Determine the integer multiple of the unit cell vector the position occupies.
    ! 3. Subtract this from the non-projected position to obtain the minimum image.
    minimumImage = position
    do id = 1, 3
       units = anint(dot_product(position, this%grid%unitCellUnitVectorsR(id, :)) &
            / this%grid%unitCellLengths(id))
       minimumImage = minimumImage - units * this%grid%unitCellVectorsR(id, :)
    end do
#else
    _REAL_ :: f(3)
