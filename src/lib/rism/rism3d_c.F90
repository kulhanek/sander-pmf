...
#include "def_time.h"
  ...
  type rism3d
     ...
     _REAL_ :: buffer = 12d0
     !> Fixed box size for 3D-RISM.  For PBC calculations, these
     !! should generally be equal. [A]
  ...
  private :: resizeBox, reallocateBox, reallocateBox_dT, &
       ...
       solve3DRISM, single3DRISMsolution, singlePeriodic3DRISMsolution, &
...
contains
  ...
  ! !> Set the unit cell of the solute.
  ! !! @param[in,out] this rism3d object.
  ! !! @param[in] unitCellDimensions Dimensions in order:
  ! !!              a, b, c, alpha, beta, gamma.
  ! subroutine rism3d_setUnitCellDimensions(this, unitCellDimensions)
  !   implicit none
  !   type(rism3d), intent(inout) :: this
  !   _REAL_, intent(in) :: unitCellDimensions(6)
  !   call rism_timer_start(this%timer)
  !   call rism3d_solute_setUnitCellDimensions(this%solute, unitCellDimensions)
  !   call rism_timer_stop(this%timer)
  ! end subroutine rism3d_setUnitCellDimensions
  ...
  subroutine rism3d_calculateSolution(this, ksave, kshow, maxSteps, tolerance)
...
#ifdef RISM_DEBUG
    write(0,*) "(1)"
    write(0,*) "RATU", this%mpirank, this%solute%numAtoms, this%grid%localDimsR, this%grid%waveNumberArraySize
    call flush(0)
#endif /*RISM_DEBUG*/
...
#if 0
    ! Offset solute by whole multiples of unit cell vectors.
    ! offset(1) = 1.5
    ! offset(2) = 4.8
    ! offset(3) = 2.3
    offset(1) = 1
    ...
    do iu = 1, this%solute%numAtoms
       do id = 1, 3
          ! this%solute%position(:, iu) = this%solute%position(:, iu) &
          !      + offset(id) * this%grid%unitCellVectorsR(id, :)
          this%solute%position(id, iu) = this%solute%position(id, iu) &
               + offset(id)
       end do
    end do
#endif
    ...
    ! this%potential%dcfLongRangeAsympR = 0
    ! this%potential%dcfLongRangeAsympK = 0
    ! this%potential%tcfLongRangeAsympR = 0
    ! this%potential%tcfLongRangeAsympK = 0
    ! this%potential%huvk0 = 0
...
#  endif /*RISM_DEBUG*/
    ...
    if (this%periodic) then
       ...
       ! Create the smallest box which fits the unit cell (origin
       ! should not matter). The volume not contained in the unit cell
       ! will be discarded later.

       !TODO: Use example for varbox to ensure dimensions are prime
       ! factorable.

       ! The absolute values of the unit cell angle contribution
       ! expand the box sufficiently to contain the unit cell without
       ! the box leaving the positive octant of the coordinate system.
       ! Since it does not matter where the box is positioned relative
       ! to the solute in a periodic system, this works fine.

       ! ! Protect against roundoff errors in trigonometric functions
       ! ! resulting in the box shape being distorted.
       ! if (abs(cos(this%grid%unitCellAngles(2))) > 1.0E-10) then
       !    boxlen(1) = this%grid%unitCellLengths(1) &
       !         + abs(this%grid%unitCellLengths(3) * cos(this%grid%unitCellAngles(2)))
       ! else
       !    boxlen(1) = this%grid%unitCellLengths(1)
       ! end if
       ! ! boxlen(1) = ceiling(boxlen(1))
       ! ! boxlen(2) = ceiling(this%grid%unitCellLengths(2))
       ! boxlen(2) = this%grid%unitCellLengths(2)
       ! if (abs(sin(this%grid%unitCellAngles(2))) > 1.0E-10) then
       !    boxlen(3) = abs(this%grid%unitCellLengths(3) * sin(this%grid%unitCellAngles(2)))
       ! else
       !    call rism_report_error( &
       !         "Unit cell beta angle cannot be very close to 0 or 180 "// &
       !         "degrees to prevent box collapse.")
       ! end if
       ! ! boxlen(3) = ceiling(boxlen(3))
       ...
       ! Periodic case has symmetry of opposite faces so x_0 = x_N on
       ! each axis, but FFTW cannot (and should not) assume such a
       ! symmetry so both x_0 and x_N must be stored.
       ! ngr = ceiling(boxlen / this%grid%spacing) + 1
       ngr = ceiling(boxlen / this%grid%spacing)

       this%grid%spacing = boxlen / ngr

#if 0
       print *, 'angles:', this%grid%unitCellAngles, 'box calc:', &
            this%grid%unitCellLengths(3) * cos(this%grid%unitCellAngles(2))
            !this%grid%unitCellLengths(1) &
            !+ this%grid%unitCellLengths(3) * cos(this%grid%unitCellAngles(2))
       print *, 'box length:', boxlen
       print *, 'grid points:', ngr
       print *, 'grid spacing:', this%grid%spacing
#endif
       ...
       ! this%mpisize > 1.  This former is done for the sake of FFT
       ! libraries.
       ...
       ! this%grid%spacing = boxlen / (ngr - 1)
       this%grid%spacing = boxlen / ngr
  ...
  subroutine reallocateBox(this, ngr, grdspc)
    ...
    this%electronMap => safemem_realloc(this%electronMap, &
         this%grid%globalDimsR(1), this%grid%globalDimsR(2), this%grid%globalDimsR(3), &
         this%solvent%numAtomTypes, o_preserve = .false.)
...
#include "def_time.h"
    ...
    do istep = 1, maxSteps
       ! One solution of 3D-RISM and closure relation.
       call timer_start(TIME_R1RISM)
       if (this%periodic) then
          call singlePeriodic3DRISMsolution(this, residual, converged, tolerance)
       else
       ...
       if (kshow /= 0 .and. this%mpirank == 0 .and. this%verbose >= 2) then
          if (converged .or. mod(istep, kshow) == 0 .or. &
             ...
             call rism_report_message('(a,i5,5x,a,1pg10.3,5x,a,i3)', &
                  ' Step=', istep, 'Resid=', residual, 'IS=', getCurrentNVec(this%mdiis_o))
             call rism_report_flush()
          end if
       end if
       ...
       if (ksave /= 0 .and. first) then
          if (converged .or. ksave > 0 .and. mod(istep, ksave) == 0) then
             call writeRestartFile(cuvsav, this%cuv(:, :, :, :), &
                  this%grid%totalLocalPointsR, this%solvent%numAtomTypes)
          end if
       end if
#endif /*MPI*/
    ...
    if (.not. converged) then
       call rism_report_error('(a,i5)','RXRISM: reached limit # of relaxation steps: ', maxSteps)
    end if
  ...
  !> One relaxation step for the 3D-RISM equation with the HNC closure,
  ...
  !! @param[in,out] residual MDIIS extrapolated residual for this step.
  ...
  subroutine singlePeriodic3DRISMsolution(this, residual, converged, tolerance)
    use rism3d_fft_c
...
#include "def_time.h"
    ...
    integer ::  ig1, iga, iv, iv1, iv2, igx, igy, igz, igr, igk
#ifdef FFW_THREADS
    ...
    integer :: id
    _REAL_ :: centerOfMass(3) = (/ 0, 0, 0 /)

    call rism_timer_start(this%single3DRISMsolutionTimer)

    ! DCF is loaded in guv array so it can be handled passed to FFTW.
    !TODO: This memory copy could be avoided if cuv was simply reinterpreted as a 1D-array.
#if defined(MPI)
    do iv = 1, this%solvent%numAtomTypes
       do igz = 1, this%grid%localDimsR(3)
          do igy = 1, this%grid%localDimsR(2)
             do igx = 1, this%grid%localDimsR(1)
                igk = igx + ((igy - 1)  + (igz - 1) * this%grid%localDimsR(2)) &
    ...
    do iv = 1, this%solvent%numAtomTypes
       ...
       ! Zero out extra space.
       this%guv(this%grid%totalLocalPointsR + 1:this%grid%totalLocalPointsK, iv) = 0d0
    end do
#endif

    ! FFT the DCF into k-space.
    ...
    ! Solve the 3D-RISM equation for the TCF in k-space.
    do iv1 = 1, this%solvent%numAtomTypes
       do ig1 = 1, this%grid%totalLocalPointsK
          ...
          do iv2 = 1, this%solvent%numAtomTypes
             this%huv(ig1, iv1) = this%huv(ig1, iv1) &
                  + this%guv(ig1, iv2) &
                  * this%xvva(iga + (iv2 - 1) * this%grid%waveNumberArraySize &
                  + (iv1 - 1) * this%grid%waveNumberArraySize * this%solvent%numAtomTypes)
          end do
       end do
    end do

    ! Inverse FFT the TCF into r-space.
    call timer_start(TIME_RISMFFT)
    call rism_timer_start(this%fftTimer)
#if defined(MPI)
    ...
    do iv = 1, this%solvent%numAtomTypes
       do igz = 1, this%grid%localDimsR(3)
          do igy = 1, this%grid%localDimsR(2)
             do igx = 1, this%grid%localDimsR(1)
                igr = igx + (igy - 1) * this%grid%localDimsR(1) &
                     + (igz - 1) * this%grid%localDimsR(2) * this%grid%localDimsR(1)
#if defined(MPI)
                ...
                igk = igr
#endif /*defined(MPI)*/
                this%cuvres(igr, iv) = this%guv(igk, iv) - 1d0 - this%huv(igk, iv)
             end do
          end do
       end do
    end do
  ...
  end subroutine singlePeriodic3DRISMsolution
...
#include "def_time.h"
    ...
    if (this%solute%charged) then
       do iv = 1, this%solvent%numAtomTypes
          do ig1 = 1, this%grid%totalLocalPointsK
             this%guv(ig1, iv) = this%guv(ig1, iv) - this%solvent%charge(iv) * this%potential%dcfLongRangeAsympK(ig1)
          end do
       end do
    end if
...
#include "def_time.h"
  ...
  !! smearing.
  !! @param[in] this rism3d object.
  !! @param[in] electronDensityRDF Solvent 1D electron density map.
  ...
  subroutine createElectronDensityMap(this, electronDensityRDF, &
       electronRDFGridSpacing, totalSolventElectrons, electronMap)
    implicit none
    type(rism3d), intent(in) :: this
    _REAL_, intent(in) :: electronDensityRDF(:)
    ...
    !FIXME: What unit conversions are necessary?
    _REAL_, parameter :: UNITS = 55.34 * 6.02214129E-4
    ! Value near zero.
    ...
    do igz = -numSmearGridPoints(3), numSmearGridPoints(3)
       ...
       do igy = -numSmearGridPoints(2), numSmearGridPoints(2)
          ...
          do igx = -numSmearGridPoints(1), numSmearGridPoints(1)
             ...
             if (rdfIndex <= size(electronDensityRDF)) then
                electronRDFSum = electronRDFSum + &
                     electronDensityRDF(rdfIndex) * this%grid%voxelVolume
             end if
          end do
       end do
    end do
    ...
    do igzCenter = 0, this%grid%globalDimsR(3) - 1
       ...
       do igyCenter = 0, this%grid%globalDimsR(2) - 1
          ...
          do igxCenter = 0, this%grid%globalDimsR(1) - 1
             ...
             !TODO: The second index into guv assumes oxygen at that
             ! location. Generalizing will require some effort.
             if (this%guv(igCenter, 1) > nearZero) then
                ! Center grid.
!$omp critical
                electronMap(igxCenter + 1, igyCenter + 1, igzCenter + 1) = &
                     electronMap(igxCenter + 1, igyCenter + 1, igzCenter + 1) + &
                     numElectronsAtGridCenter * &
                     this%guv(igCenter, 1) * UNITS
!$omp end critical
                   ...
                   do igy = igyCenter - numSmearGridPoints(2), igyCenter + numSmearGridPoints(2)
                      ...
                      do igx = igxCenter - numSmearGridPoints(1), igxCenter + numSmearGridPoints(1)
                         ...
                         if (distance .le. cutoff) then
                            ...
                            rdfIndex = ceiling(distance / electronRDFGridSpacing)
                            if (rdfIndex .le. size(electronDensityRDF)) then
!$omp critical
