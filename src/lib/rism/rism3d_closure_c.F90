...
contains
  ...
  !! @param[in] huv Site-site total correlation function.
  !! @param[in] cuv Site-site direct correlation function.
  function rism3d_closure_excessChemicalPotential(this, huv, cuv) result(excessChemicalPotential)
    implicit none
    type(rism3d_closure), intent(in) :: this
    _REAL_, intent(in) :: huv(:,:), cuv(:,:,:,:)
  ...
  function rism3d_closure_excessChemicalPotential_tot_map(this, huv, cuv) result(excessChemicalPotential)
    implicit none
    type(rism3d_closure), intent(in) :: this
    _REAL_, intent(in) :: huv(:,:),cuv(:,:,:,:)
  ...
  function rism3d_closure_excessChemicalPotential_ijk(this, huv, cuv,ijk) result(excessChemicalPotential)
    implicit none
    type(rism3d_closure), intent(in) :: this
    _REAL_, intent(in) :: huv(:,:),cuv(:,:,:,:)
  ...
  function rism3d_closure_excessChemicalPotentialPCPLUS_tot_map(this, huv, cuv)&
    ...
    do iz = 1, this%grid%localDimsR(3)
       do iy = 1, this%grid%localDimsR(2)
          do ix = 1, this%grid%localDimsR(1)
             excessChemicalPotential(ix,iy,iz) = rism3d_closure_excessChemicalPotentialPCPLUS_ijk&
                  (this,huv,cuv,(/ix,iy,iz/))
          end do
       end do
    end do
  ...
  function rism3d_closure_excessChemicalPotentialUC_tot_map(this, huv, cuv,coeff)&
    ...
    do iz = 1, this%grid%localDimsR(3)
       do iy = 1, this%grid%localDimsR(2)
          do ix = 1, this%grid%localDimsR(1)
             excessChemicalPotential(ix,iy,iz) = rism3d_closure_excessChemicalPotentialUC_ijk&
  ...
  !!   ijk  : 3d-grid index
  !!   iv   : solvent site
  function rism3d_closure_excessChemicalPotentialGF_IJK(this, huv, cuv,ijk,iv) result(excessChemicalPotential)
    implicit none
    type(rism3d_closure), intent(in) :: this
    _REAL_, intent(in) :: huv(:,:),cuv(:,:,:,:)
...
#endif /*defined(MPI)*/
    tuv = huv(igk,iv) - cuv(ix,iy,iz,iv)
    excessChemicalPotential = - (1.d0+0.5d0*huv(igk,iv))*cuv(ix,iy,iz,iv)*this%potential%solvent%density(iv)
  ...
  !!   huv  : site-site total correlation function
  !!   cuv  : site-site direct correlation function
  function rism3d_closure_aexcessChemicalPotentialGF(this, huv, cuv) result(excessChemicalPotential)
    implicit none
    type(rism3d_closure), intent(inout) :: this
    _REAL_, intent(in) :: huv(:,:),cuv(:,:,:,:)
    _REAL_ :: excessChemicalPotential(this%solvent%numAtomTypes)
    ...
    if (.not.this%solute%charged) then
       excessChemicalPotential = rism3d_closure_excessChemicalPotentialGF(this,huv,cuv)
       return
    end if
    ...
    !
    call rism3d_potential_int_h2_hc(this%potential,excessChemicalPotentialh2lr,excessChemicalPotentialhclr)
    !
    !Short-range part
    !
    !TODO: The use of long range asymptotics may need to be corrected for periodic case.
    huvlr=0d0
    excessChemicalPotential = 0.d0
    do iv=1,this%solvent%numAtomTypes
       do iz=1,this%grid%localDimsR(3)
       ...
       excessChemicalPotential(iv) =  excessChemicalPotential(iv)*this%grid%voxelVolume
    end do
    excessChemicalPotential = this%solvent%density*(excessChemicalPotential-excessChemicalPotentialhclr/2d0)
  ...
  !!OUT:
  !!    The total excess chemical potential.  It is not site decomposible.
  function rism3d_closure_aexcessChemicalPotentialPCPLUS(this, huv, cuv) result(excessChemicalPotential)
    implicit none
    type(rism3d_closure), intent(inout) :: this
    _REAL_, intent(in) :: huv(:,:),cuv(:,:,:,:)
    _REAL_ :: excessChemicalPotential
  ...
  function rism3d_closure_solvationEnergy(this, huv_dT, cuv_dT, huv, cuv) result(solvationEnergy)
    use rism_util, only : gaussquad_legendre, checksum
    implicit none
    type(rism3d_closure), intent(inout) :: this
...
!!$                  igk = ix + (iy-1)*this%grid%localDimsR(1) + (iz-1)*this%grid%localDimsR(2)*this%grid%localDimsR(1)
!!$#endif /*defined(MPI)*/
!!$                  ene(iv) = ene(iv) + guv(igk,iv)*this%potential%uuv(ix,iy,iz,iv)
  ...
  function rism3d_closure_solvPotEne_ijk (this,guv,ijk,iv) result(ene)
    ...
    igk = ix + (iy-1)*this%grid%localDimsR(1) + (iz-1)*this%grid%localDimsR(2)*this%grid%localDimsR(1)
#endif /*defined(MPI)*/
    ene = guv(igk,iv)*this%potential%uuv(ix,iy,iz,iv)*this%potential%solvent%density(iv)
...
#endif /*defined(MPI)*/
                H(iv) = H(iv) + (guv(igk, iv) - 1d0) &
                     - this%potential%solvent%charge_sp(iv) * this%potential%tcfLongRangeAsympR(ig)
             end do
          end do
       end do
    end do
...
!!$    end function rism3d_closure_aDCFintegral
  ...
  subroutine rism3d_closure_force(this, ff, guv, periodicPotential)
    ...
    if (periodicPotential == '') then
       ...
       if (periodicPotential == 'pme') then
          ...
          ! call ewaldSumShortRangeForce(this%potential, ff, guv)
       end if
    end if

#if 0    
    atomRange = min(10, this%solute%numAtoms / 2)
    print *, ff(:, :atomRange)
    do atom = this%solute%numAtoms - atomRange, this%solute%numAtoms
       print *, ff(:, atom)
    end do
#endif    
          ...
          do igy = first(2), last(2)
             ...
             do igx = first(1), last(1)
                ...
                ig = 1 + igx + igy * (this%grid%globalDimsR(1) + 2) + &
                     igz * this%grid%globalDimsR(2) * (this%grid%globalDimsR(1) + 2)
#else
                ...
                !TODO: Branch prediction should handle this fine, but
                ! benchmarking can confirm. Could use code duplication
                ! preprocessor statement or maybe something similar in
                ! case branch prediction doesn't work.
                if (periodic) dX = minimumImage(this%potential, dX)
  ...
  subroutine lennardJonesForcePeriodic(this, ff, guv)
    ...
    _REAL_, intent(inout) :: ff(3, this%solute%numAtoms) !> Force applied to each atom.
    _REAL_, intent(in) :: guv(:, :) !< Site-site pair correlation function.
          ...
          do igy = 1, this%grid%localDimsR(2)
             ...
             do igx = 1, this%grid%localDimsR(1)
                ...
                ig = 1 + (igx - 1) + (igy - 1) * this%grid%globalDimsR(1) + &
                     (igz - 1) * this%grid%globalDimsR(2) * this%grid%globalDimsR(1)
#endif /*defined(MPI)*/
                ...
                if (soluteDistanceSquared < this%cutoff2) then
                   ...
                   ! do id = 1, 3
                   !    ff(id, iu) = ff(id, iu) + dUlj_dr * solutePosition(id)
                   ! end do
                end if
             end do
          end do
...
#ifdef RISM_DEBUG
    write(6, * ) "FORCE_COULOMB", this%potential%cutoff2, sqrt(this%potential%cutoff2)
    call flush(6)
#endif /*RISM_DEBUG*/
    smstart = mod(this%grid%offsetR(3), 2)
    qutemp(1:this%solute%numAtoms) = this%solute%charge(1:this%solute%numAtoms)! *AMBER_ELECTROSTATIC
    qvtemp(1:this%solvent%numAtomTypes) = this%solvent%charge(1:this%solvent%numAtomTypes)! *AMBER_ELECTROSTATIC
    offset = this%grid%spacing(3) * this%grid%offsetR(3)
    ! Calculate the number of grid point necessary to cover this
    ! cutoff range. In case of a large cutoff, we need to protect
    ! against integer overflow.
          ...
          do igy = 0, this%grid%localDimsR(2) - 1, 2
             ...
             do igx = 0, this%grid%localDimsR(1) - 1, 2
                ...
                ig = 1 + igx + igy * (this%grid%localDimsR(1) + 2) + &
                     igz * this%grid%localDimsR(2) * (this%grid%localDimsR(1) + 2)
#else
  ...
  subroutine ewaldSumForce(this, ff, guv)
       ...
       do igz = 0, this%grid%globalDimsR(3) - 1
          ...
          do igy = 0, this%grid%globalDimsR(2) - 1
             ...
             do igx = 0, this%grid%globalDimsR(1) - 1
                ...
                ! if (ig == 2) print *, this%grid%waveVectors(:, ig), solutePosition
                ! if (ig > this%grid%totalLocalPointsK - 30) &
                !      print *, this%grid%waveVectors2(ig), exp(-smear2_4 * this%grid%waveVectors2(ig))
                ! if (exp(-smear2_4 * this%grid%waveVectors2(ig)) /= 0) &
                !      print *, exp(-smear2_4 * this%grid%waveVectors2(ig))
    ...
    !TODO: Add in short-range Ewald sum.
  end subroutine ewaldSumForce


  ! There is no direct relation between the potential created by the
  ! periodic solute and the resulting solvent charge distribution
  ! obtained from solving the 3D-RISM equations, thus the the
  ! potential cannot be reused to calculate the forces.
  !TODO: Clean this up after the corresponding potential is cleaned
  ! up.
  subroutine particleMeshEwaldForce(this, ff, guv)
    ...
    do iu = 1, this%solute%numAtoms
       ...
       !TODO: pmemd does this at the end of the grad_sum routine. Why?
       ! Part of chain rule? Probably projects forces from reciprocal
       ! to real space.
       do id = 1, 3
          ff(id, iu) = ff(id, iu) - &
               dot_product(this%grid%unitCellVectorsK(id,:), potGrad)
               ! dot_product(this%grid%unitCellVectorsR(id,:), potGrad)
       end do
       ! ff(:, iu) = ff(:, iu) - potGrad
    end do

    ! No short-range correction is needed since the solvent is treated
    ! as point particles, not Gaussian charges.
             ...
             do iu = 1, this%solute%numAtoms
                ...
                do iv = 1, this%solvent%numAtomTypes
                   ...
                   do id = 1, 3
                      ...
                      ff(id, iu) = &
                           ff(id, iu) - dU_dR(id) * this%solvent%density(iv) * guv(ig, iv)
                   end do
                end do
             end do
          end do
