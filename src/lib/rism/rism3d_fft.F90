...
#  endif /*RISM_DEBUG*/
  ...
  subroutine rism3d_fft_new(this,&
    ...
    this%grid => grid
    ncmplx = (/this%grid%globalDimsK(1) / 2, this%grid%globalDimsK(2), this%grid%globalDimsK(3)/)
    this%narray = ubound(inr, 2)
    if (this%aligned == FFT_ALIGNED) then
       !aligned memory
       sites_per_transform = this%narray
#ifdef MPI
       this%wrk_trans=>safemem_realloc(this%wrk_trans, this%narray*product(this%grid%globalDimsR),.false.)
#endif
    else
       !unaligned memory
       sites_per_transform = 1
...
!!$           comm, FFTW_ESTIMATE)
#else
    this%wrk => safemem_realloc(this%wrk, 2*product(this%grid%globalDimsR(2:3)),.false.)
...
#  endif
       ...
       if (this%aligned == FFT_ALIGNED) then
          do i = 1, this%narray
             do j = 1, nk
                this%wrk_trans( &
                     1 + (i - 1) * 2 + (j - 1) * this%narray * 2) = real(datak(j, i))
                this%wrk_trans( &
                     2 + (i - 1) * 2 + (j - 1) * this%narray * 2) = aimag(datak(j, i))
             end do
          end do
          ...
          ! do iarray = 1, ubound(datar, 2)
          !    do ir = 1, ubound(datar, 1)
          !       write(0,*) "ouk", iarray, ir, -1, datar(ir, iarray)/product(this%grid%globalDimsR)
          !    end do
          ! end do
...
#  endif
       ! do iarray = 1, ubound(datar, 2)
       !    write(0,*) "out", -1, sum(data(1:this%grid%globalDimsR(1),:,:,iarray))
       ! end do
    end if
