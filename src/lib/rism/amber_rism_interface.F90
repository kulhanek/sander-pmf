...
#  endif /*defined(MPI)*/
       ...
       if (iv == 1 .and. len_trim(electronMapFile) /= 0) then
          call getenv('AMBERHOME', amberhome)
          call readRDF1D(trim(amberhome)//"/dat/rism3d/electron_rdf/water_O.csv", electronRDF, electronRDFGridSpacing)
          !TODO: Z = 10 for water is hard coded for now.
          call createElectronDensityMap(this, electronRDF, electronRDFGridSpacing, 10, &
               this%electronMap(:, :, :, 1))
          call writeVolume(trim(electronMapFile)//suffix, this%electronMap(:, :, :, 1), &
               this%grid, this%solute, mpirank, mpisize, mpicomm)
          if (safemem_dealloc(electronRDF) /= 0) then
             call rism_report_error("rism_writeVolumetricData: Deallocate of electron RDF failed.")
          end if
       end if
    end do
...
#ifdef SANDER
    ! For AmberTools17, disable periodic RISM: not quite mature enough,
    !   especially in parallel:
    if (periodicPotential /= '' ) then
       write(6,'(a)') 'periodic RISM is not supported'
       call mexit(6,1)
    end if
#endif
