...
module rism3d_grid_c
...
#if 0
  !> Periodic solute data defining the unit cell (not relevant to
  !! infinite dilution case).
  type rism3d_UnitCell
     !> Number of residues in this solute supercell.
     ! integer :: numResidues = 0
     !> Which residue each atom is associated with.
     !! TODO: Perhaps should be a map using residue names?
     ! integer, pointer :: residue(:) => NULL()
     !> Dimensions of the parallelogram containing the periodic
     !! unit cell (dimension, residue). [A]
     ! _REAL_, pointer :: dimensions(:,:) => NULL()
     _REAL_ :: lengths(3)
     !> Angle between the XY and ZY planes which defines the
     !! inclination of the unit cell parallelogram.
     !! Angles can differ between residues. [degrees]
     ! _REAL_, pointer :: xzAngle(:) => NULL()
     !> Angles between the planes YX and ZX, XY and ZY, and XZ and YZ
     !! respectively.  These angles are commonly referred to as alpha,
     !! beta, and gamma angles.
     _REAL_ :: angles(3)
  end type rism3d_UnitCell
#endif
  ...
  type rism3d_grid
     ...
     !TODO: Convert to unit cell data structure (above).
     !TODO: Swap indices to be (vector, axis) since this is used
     ! throughout rest of codebase.
     !> Unit cell for this solute.
     ! type(rism3d_UnitCell) :: unitCell
...
#if 0
    this%unitCellUnitVectorsR(1, :) = (/ 1, 0, 0 /)
    this%unitCellUnitVectorsR(2, :) = (/ 0, 1, 0 /)
    this%unitCellUnitVectorsR(3, :) = (/ 0, 0, 1 /)

    this%unitCellUnitVectorsR(2, :) = &
         matmul(rotationMatrixFromEulerAngles( &
         0d0, 0d0, -(this%unitCellAngles(3) - PI / 2)), &
         this%unitCellUnitVectorsR(2, :))

    this%unitCellUnitVectorsR(3, :) = &
         matmul(rotationMatrixFromEulerAngles( &
         -(this%unitCellAngles(1) - PI / 2), 0d0, 0d0), &
         this%unitCellUnitVectorsR(3, :))
    
    this%unitCellUnitVectorsR(3, :) = &
         matmul(rotationMatrixFromEulerAngles( &
         0d0, -(this%unitCellAngles(2) - PI / 2), 0d0), &
         this%unitCellUnitVectorsR(3, :))
#else
...
#if 0
    print *, "Unit cell dimensions: ", unitCellDimensions
    print *, "Unit cell lengths and angles: ", this%unitCellLengths, this%unitCellAngles
    call printUnitCellVectorProperties(this%unitCellUnitVectorsR, "Unit cell unit")
    call printUnitCellVectorProperties(this%unitCellVectorsR, "Unit cell")
    call printUnitCellVectorProperties(this%voxelVectorsR, "Voxel")
#endif
